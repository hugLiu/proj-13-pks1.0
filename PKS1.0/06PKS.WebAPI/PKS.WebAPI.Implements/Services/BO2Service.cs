...
namespace PKS.WebAPI.Services
{
    ...
    public class BO2Service : IBO2Service, ISingletonAppService
    {
        ...
        {
            JObject jProject = JObject.Parse(request.Fields.ToJson());
            jProject.Add(new JProperty("_id", 0));

            var result = await docs
                        .Find(new JsonFilterDefinition<MongoDB.Bson.BsonDocument>(request.Query.ToJson()))
                        .Sort(new JsonSortDefinition<MongoDB.Bson.BsonDocument>(request.Sort.ToJson()))
                        .Project(new JsonProjectionDefinition<MongoDB.Bson.BsonDocument>(jProject.ToJson()))
                        .Skip(request.Skip)
                        .Limit(request.Limit)
                        .ToListAsync();
        ...
        public async Task<object> SaveBOsAsync(List<BO2> bos)
        {
            ...
            foreach (var bo in bos)
            {
                ...
                using (var cursor = (await this.BOBsonDocumentCollection.FindAsync(filter)))
                {
                    ...
                    if (bo.Location != null)
                    {
                        oldDoc["location"] = MongoDB.Bson.BsonValue.Create(bo.Location);
                    }
