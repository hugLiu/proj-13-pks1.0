var pksGlobalStore = {
    ...
    //初始化
    init: function (apiSiteUrl, apiServiceToken) {
        this.config.apiSiteUrl = this.normalizeUrl(apiSiteUrl);
        this.config.apiServiceUrl = this.config.apiSiteUrl + "api/";
        this.config.apiServiceToken = apiServiceToken;
        this.initConsole();
        this.setJQueryAjaxSetup(apiServiceToken);
        this.logService.init.call(this.logService, this, this.config.apiServiceUrl);
    },
    //jQuery全局ajax配置
    setJQueryAjaxSetup: function (apiServiceToken) {
        if (!window.$) return;
        //$.ajaxSetup({
        //    headers: {
    ...
    ajaxPost: function (url, data, success) {
        ...
        ajaxOption.headers = { Authorization: "Bearer " + store.config.apiServiceToken };
        $.ajax(ajaxOption);
    },
    //获得元素标题
    ...
    logService: {
        ...
        onError: function (errorEvent) {
            ...
            if (store.config.apiServiceToken) {
                logData.principal = "Token:" + store.config.apiServiceToken;
            }
            //logData.exSource = "";
            ...
            store.logService.logError.call(store.logService, logData);
        }
    },
    //根据bo，pt,date进入展示页面
    goToDetailService: {
